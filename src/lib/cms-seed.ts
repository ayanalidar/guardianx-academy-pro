/**
 * Source-of-truth CMS seed content for the entire platform.
 *
 * This module is imported by:
 *   - prisma/seed-cms.ts  (CLI: `bun run prisma/seed-cms.ts`)
 *   - /api/admin/site-content/seed  (POST: lets admins re-seed from the
 *     Content Studio UI without shell access)
 *
 * Shape: an array of { page, section, key, value } items. `value` may
 * be any JSON-serializable type (string | string[] | object[] | object).
 *
 * Keeping the seed content in a shared module (instead of inline in
 * prisma/seed-cms.ts) means the API route can upsert the same defaults
 * with the request's authenticated Prisma client + updatedBy audit
...
// Backwards-compat alias used by the inline array declarations below
// (originally `type ContentItem` in prisma/seed-cms.ts).
type ContentItem = CmsSeedItem
